...
namespace TierraSanta.Controllers
{
    ...
    {
		public List<EMPRESAS> Items { get; set; }
    ...
    public class EmpresasController : Controller
    {
        ...
        {
		//
			var viewModel = new EMPRESASIndexViewModel();            

            //if (Search == null || Search.Equals(""))
            //{
				var pager = new Pager(db.EMPRESAS.Count(), page);
                viewModel.Items = db.EMPRESAS
    ...
    //        else
    //        {
				//var pager = new Pager(db.EMPRESAS.Where(c => c.AgregarVariableAbuscar.Contains(Search)).Count(), page);
    //            viewModel.Items = db.EMPRESAS.Where(c => c.AgregarVariableAbuscar.Contains(Search))
    //                    .OrderBy(c => c.EMPRESASID)
    //                    .Skip((pager.CurrentPage - 1) * pager.PageSize)
    //                    .Take(pager.PageSize).ToList();
				//viewModel.Pager = pager;
    ...
    //        }
        ...
        public ActionResult Create([Bind(Include = "idempresa,idubicacion,idpersoneria,idregimen,idtipo,idmoneda,idcomision,idusuario,control,parametros,ruc,razonsocial,direccion,Abreviatura,fechainicio,estado,logotipo,fondo")] EMPRESAS eMPRESAS)
        {
            List<EMPRESAS> f = db.EMPRESAS.ToList();
            if (f.Count == 0) { eMPRESAS.idempresa = "01"; }
            else { eMPRESAS.idempresa = getidempresa(Convert.ToInt32(f.Last().idempresa)); }
            eMPRESAS.idubicacion = "01040101";
            eMPRESAS.idpersoneria = "200002";
            eMPRESAS.idregimen = "100002";
            eMPRESAS.idtipo = "040001";
            eMPRESAS.idmoneda = "020001";
            eMPRESAS.idcomision = "120001";
            eMPRESAS.idusuario = "0001";
            eMPRESAS.control = "11111111111110000000";
            eMPRESAS.parametros = "0002210N0101006101000012010000";
            eMPRESAS.fechainicio = DateTime.Now;
            eMPRESAS.estado = "1";
            eMPRESAS.logotipo = "";
            eMPRESAS.fondo = "";
            if (ModelState.IsValid)
            {
                //eMPRESAS.Creado = DateTime.Now;
                //eMPRESAS.Modificado = DateTime.Now;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "idempresa,idubicacion,idpersoneria,idregimen,idtipo,idmoneda,idcomision,idusuario,control,parametros,ruc,razonsocial,direccion,Abreviatura,fechainicio,estado,fechacambio,logotipo,fondo")] EMPRESAS eMPRESAS)
        {
            eMPRESAS.fechacambio = DateTime.Now;
            eMPRESAS.logotipo = "";
            eMPRESAS.fondo = "";
            if (ModelState.IsValid)
            {
