...
namespace WebTS2.Controllers
{
    ...
    public class ActividadesManodeObraIndexViewModel
    {
        public List<TablaActividades> Items { get; set; }
        public Pager Pager { get; set; }
    }

    public class ActividadesManodeObraController : Controller
    {
        private EntitiesTierraSanta db = new EntitiesTierraSanta();


        // GET: ActividadesManodeObra
        public ActionResult Index(int? page, String Search)
        {
            //
            var viewModel = new ActividadesManodeObraIndexViewModel();

            if (Search == null || Search.Equals(""))
            {
                var pager = new Pager(db.TablaActividades.Where(x => x.idparent == 1 && x.abreviatura != "").Count(), page);
                viewModel.Items = db.TablaActividades.Where(x => x.idparent == 1 && x.abreviatura != "").Include(t => t.TablaCultivos).Include(t => t.TablaActividades2)
            ...
            {
                var pager = new Pager(db.TablaActividades.Where(x => x.idparent == 1 && x.abreviatura != "" && x.descripcion.Contains(Search)).Count(), page);
                viewModel.Items = db.TablaActividades.Where(x => x.idparent == 1 && x.abreviatura != "" && x.descripcion.Contains(Search)).Include(t => t.TablaCultivos).Include(t => t.TablaActividades2)
                    .OrderBy(c => c.idactividades)
                    .Skip((pager.CurrentPage - 1) * pager.PageSize)
                    .Take(pager.PageSize).ToList();
                viewModel.Pager = pager;
        ...
        // GET: ActividadesManodeObra/Details/5
        ...
        // GET: ActividadesManodeObra/Create
        public ActionResult Create()
        {
            ...
            ViewBag.idparent = new SelectList(db.TablaActividades.Where(x => x.idparent == 1 && x.abreviatura == ""), "idparent", "descripcion");
            return View();
        }

        // POST: ActividadesManodeObra/Create
        ...
        public ActionResult Create([Bind(Include = "idactividades,idparent,idempresa,idusuario,descripcion,abreviatura,unimedida,costo1,fechacreacion,fechacambio")] TablaActividades tablaActividades)
        {
            ...
            ViewBag.unimedida = new SelectList(db.TablaCultivos, "pktablacultivos", "idempresa", tablaActividades.unimedida);
            ViewBag.idparent = new SelectList(db.TablaActividades, "idactividades", "idempresa", tablaActividades.idparent);
            return View(tablaActividades);
        }

        // GET: ActividadesManodeObra/Edit/5
        ...
        // POST: ActividadesManodeObra/Edit/5
        // Para protegerse de ataques de publicación excesiva, habilite las propiedades específicas a las que desea enlazarse. Para obtener 
        // más información vea https://go.microsoft.com/fwlink/?LinkId=317598.
        [HttpPost]
        [ValidateAntiForgeryToken]
        ...
        // GET: ActividadesManodeObra/Delete/5
        ...
        // POST: ActividadesManodeObra/Delete/5
        ...
        public ActionResult ReportExcel()
        {
            using (var package = new ExcelPackage())
            {
                ...
                List<TablaActividades> list = db.TablaActividades.Where(x => x.idparent == 1).Include(t => t.TablaCultivos).Include(t => t.TablaActividades2).ToList();
                int pos = 4;
