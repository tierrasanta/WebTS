...
namespace WebTS2.Controllers
{
    ...
    public class ActividadesMaquinariaController : Controller
    {
        private EntitiesTierraSanta db = new EntitiesTierraSanta();


        // GET: ActividadesMaquinaria
        public ActionResult Index(int? page, String Search)
        {
            ...
            {
                var pager = new Pager(db.TablaActividades.Where(x => x.idparent == 2 && x.abreviatura != "" && x.descripcion.Contains(Search)).Count(), page);
                viewModel.Items = db.TablaActividades.Where(x => x.idparent == 2 && x.abreviatura != "" && x.descripcion.Contains(Search)).Include(t => t.TablaCultivos).Include(t => t.TablaActividades2)
                    .OrderBy(c => c.idactividades)
                    .Skip((pager.CurrentPage - 1) * pager.PageSize)
                    .Take(pager.PageSize).ToList();
                viewModel.Pager = pager;
        ...
        public ActionResult Create()
        {
            ...
            ViewBag.idparent = new SelectList(db.TablaActividades.Where(x => x.idparent == 2 && x.abreviatura == ""), "idparent", "descripcion");
            return View();
        }

        // POST: ActividadesMaquinaria/Create
        ...
        public ActionResult ReportExcel()
        {
            using (var package = new ExcelPackage())
            {
                ...
                List<TablaActividades> list = db.TablaActividades.Where(x => x.idparent == 2).Include(t => t.TablaCultivos).Include(t => t.TablaActividades2).ToList();
                int pos = 4;
